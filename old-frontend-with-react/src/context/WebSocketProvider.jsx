...
export const WebSocketProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const connectSocket = () => {
      const socketIO = io(import.meta.env.VITE_WS_URL || 'http://localhost:3000', {
  ...
  const endSession = useCallback(() => {
    if (!socket?.connected || !activeSession) {
      console.error('Cannot end session: socket disconnected or no active session');
      return false;
    }
  ...
  const sendCheckInResponse = useCallback((checkInId, response) => {
    if (!socket?.connected) {
    ...
    <SocketContext.Provider value={value}>
      ...
      {notification && (
        <NotificationPopup
          ...
          onRespond={(response) => sendCheckInResponse(notification.id, response)}
